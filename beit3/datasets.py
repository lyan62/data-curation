...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        tokenizer, num_max_bpe_tokens, task=None, items=None
    ):
...
def _make_captioning_coco_karpathy_curation_train_dataset_index(
        data_path, 
        tokenizer, 
        split=("train", "restval"), 
        split_name="train", 
):
    ### build train dataset that contain all information for curation use
    coco_karpathy_split_json_file = os.path.join(data_path, "dataset_coco.json")
    items = []
    image_counter = set()
    print("read %s" % coco_karpathy_split_json_file)
                ...
                if item["split"] in ["train", "restval"]:
                    for sent in item["sentences"]:
                        ...
                        items.append({
                                ...
                                "text_segment": token_ids,
                                "image_id": item["cocoid"], 
                                "source": "original", # add source for difference to sd
                                "tokens": sent["tokens"], # add original tokens
                                "caption_id": sent["sentid"], # add sentid
                                "sample_id": idx # add sample-id
                        })
                        idx += 1
                else:
                    items.append({
    ...
    print("Find %d images and %d image-text pairs for karpathy dataset %s split !" % \
        (len(image_counter), len(items), split_name))
    index_file = os.path.join(data_path, "coco_captioning.%s-curation.jsonl" % split_name)
    _write_data_into_jsonl(items, index_file)
    pass

def _make_captioning_flickr30k_curation_train_dataset_index(
        data_path, 
        img_dir,
        tokenizer, 
        split=("train", "restval"), 
    ...
    ### build train dataset that contain all information for curation use
    flickr30k_split_json_file = os.path.join(data_path, "dataset_flickr30k.json")
    items = []
    image_counter = set()
    print("read %s" % flickr30k_split_json_file)
    with open(flickr30k_split_json_file, mode="r", encoding="utf-8") as reader:
        data = json.loads(reader.read())
        idx = 0
        for item in data["images"]:
            if item["split"] in split:
                ...
                if image_path not in image_counter:
                    image_counter.add(image_path)
    print("Find %d images and %d image-text pairs for flickr30k dataset %s split !" % \
        (len(image_counter), len(items), split_name))
    if split_name != "train":
        filename = "flickr30k_captioning.%s.jsonl" % split_name
    else:
        filename = "flickr30k_captioning.%s-curation.jsonl" % split_name
    index_file = os.path.join(data_path, filename)
    _write_data_into_jsonl(items, index_file)
    pass

# def _build_coco_train_sample_dict(self,)
...
class CaptioningDataset(BaseDataset):
    ...
    def __init__(self, data_path, image_dir, split, transform, 
                tokenizer, num_max_bpe_tokens, task, mask_prob, items=None):
        super().__init__(
            data_path=data_path, split=split, 
            transform=transform, tokenizer=tokenizer, 
            num_max_bpe_tokens=num_max_bpe_tokens, task=task, items=items
        )
    ...
    def get_index_files(self, split, task=None):
        if split == "train":
            return (f"{task}.train-curation.jsonl", )
            # return ("coco_captioning.train-small-sd.jsonl",)
        elif split == "val":
            return (f"{task}.val.jsonl", )
        elif split == "test":
            return (f"{task}.test.jsonl", )
        elif "curation" in split:
            return (split, )
        else:
            raise RuntimeError("split %s is not found!" % split)
    ...
    def __getitem__(self, index: int):
        ...
        if "train" in self.split:
            data["caption_id"] = item.get("caption_id")
            data["sample_id"] = item.get("sample_id")
    ...
    def build_train_caption_dict(self):
        # {"image_id": {"caption_id": {"text_segment": [], "tokens": str}}
        train_caption_dict = {}
        img2path = dict()
        
        for item in self.items:
            if item["image_id"] not in train_caption_dict:
                train_caption_dict[item["image_id"]] = {item["caption_id"]: {"text_segment": item["text_segment"], "tokens":item["tokens"]}}
            else:
                train_caption_dict[item["image_id"]].update({item["caption_id"]: {"text_segment": item["text_segment"], "tokens":item["tokens"]}})
            img2path[item["image_id"]]=item["image_path"]
        return train_caption_dict, img2path
    ...
    @staticmethod
    def make_coco_captioning_curation_train_dataset_index(data_path, tokenizer):
        _make_captioning_coco_karpathy_curation_train_dataset_index(data_path, tokenizer, split=("train", "restval"), split_name="train")

    @staticmethod
    def make_flickr_captioning_curation_train_dataset_index(data_path, img_dir, tokenizer):
        _make_captioning_flickr30k_curation_train_dataset_index(data_path, img_dir, tokenizer, split=("train", "restval"), split_name="train")
        _make_captioning_flickr30k_curation_train_dataset_index(data_path, img_dir, tokenizer, split=("val", ), split_name="val")
        _make_captioning_flickr30k_curation_train_dataset_index(data_path, img_dir, tokenizer, split=("test", ), split_name="test")
...
task2dataset = {
    ...
    "flickr30k_captioning": CaptioningDataset,
...
def create_dataset_by_split(args, split, is_train=True, items=None):
    ...
    opt_kwargs = {}
    if args.task in ["coco_captioning", "nocaps", "flickr30k_captioning"]:
        opt_kwargs["mask_prob"] = args.captioning_mask_prob

    dataset = dataset_class(
        data_path=args.data_path, image_dir=args.data_path, split=split, 
        transform=transform, tokenizer=tokenizer, 
        num_max_bpe_tokens=args.num_max_bpe_tokens, 
        task=args.task, items=items, **opt_kwargs, 
    )
    if is_train:
    ...
    if split == "train" and is_train and not args.eval:
        train_caption_dict, img2path_dict = dataset.build_train_caption_dict()
        ...
        ), train_caption_dict, img2path_dict
...
def create_curated_train_dataset(args, curated_data, is_train=True):
    transform = build_transform(is_train=is_train, args=args)
    dataset_class = task2dataset[args.task]
    tokenizer = get_sentencepiece_model_for_beit3(args)
    ...
    # curated_data_dir, curated_file_name = os.path.dirname(curated_data_path), os.path.basename(curated_data_path)
   
    dataset = dataset_class(
        data_path=args.data_path, image_dir=args.data_path, split="train", 
        transform=transform, tokenizer=tokenizer, 
        num_max_bpe_tokens=args.num_max_bpe_tokens, 
        task=args.task, items=curated_data, **opt_kwargs, 
    )
