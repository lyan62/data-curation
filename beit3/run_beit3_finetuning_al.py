...
from engine_for_finetuning import train_one_epoch, get_handler, evaluate, evaluate_train
from datasets import create_downstream_dataset, create_dataset_by_split, create_curated_train_dataset
...
def get_args():
    ...
    parser.add_argument('--task', type=str, required=True, 
                        choices=['nlvr2', 'vqav2', 'flickr30k', 'coco_retrieval', 'coco_captioning', 'nocaps', 'imagenet', 'flickr30k_captioning'], 
...
def get_top_loss_samples(train_result_file, args, img2path_dict):
    ...
    for sample in rest_samples:
        sample.update({"image_path": img2path_dict[sample["image_id"]]})
    print("total number of augmenting samples in %s: %d" % (args.curation_method, len(top_loss_samples)))
    return top_loss_samples, rest_samples

def replace_top_loss_samples(top_loss_samples, train_caption_dict, original_train_caption_dict, img2path_dict):
    """
    top_loss_samples:
    {"image_id": 540186, "caption_id": 259937, "loss_val": 8.046542167663574, "caption": {"text_segment": [],"tokens": "A view of a very large bathroom with mirrored walls."}},
    train_caption_dict: {image_id: {caption_id: {"text_segment": [], "tokens": str}}
    """
    replaced_samples = []
...
def lm_replace_top_loss_samples(top_loss_samples, lm_img_cap_dict):
    ...
    replaced_samples = []
    for sample in top_loss_samples:
        cur_img_id = os.path.basename(sample["image_path"])
        if cur_img_id in lm_img_cap_dict:
            cand_captions = lm_img_cap_dict[cur_img_id]["captions"]
            if len(cand_captions) > 0:
                rep_cap_idx = random.randint(0, len(cand_captions)-1)
                rep_cap_tokens = lm_img_cap_dict[cur_img_id]["cap_tokens"][rep_cap_idx]
                source = "aug"
                lm_img_cap_dict[cur_img_id]["cap_tokens"].pop(rep_cap_idx)
                lm_img_cap_dict[cur_img_id]["captions"].pop(rep_cap_idx)
                text_segment = rep_cap_tokens
        else: 
            source = "original"
            text_segment = sample["text_segment"]
        replaced_samples.append({
            "image_id": sample["image_id"],
            "text_segment": text_segment,
            "source": source,
            "caption_id": sample["caption_id"],
            "image_path": sample["image_path"], #img2path_dict[sample["image_id"]],
            "sample_id": sample["sample_id"]
            })
...
def sort_caption_by_train_loss(train_result_file):
    ...
    for sample in sorted(train_results, key=lambda x: x["loss_val"]):
        ...
        caption_loss_dict[img_id].append((sample["caption_id"], sample["loss_val"]))
    # {image_id: [(caption_id1, loss_val1),(...)]
    return caption_loss_dict


def replaceImg_top_loss_samples(top_loss_samples, original_train_caption_dict, origin2sd, caption_dict_by_loss, img2path_dict, change_caption_by=None):
    """
    top_loss_samples:
    {"image_id": 540186, "caption_id": 259937, "loss_val": 8.046542167663574, "caption": {"text_segment": [],"tokens": "A view of a very large bathroom with mirrored walls."}},
    train_caption_dict: {image_id: {"caption_id": {"text_segment": [], "tokens": str}}
    ...
    replaced_samples = []
    cnter = 0
    for idx, sample in enumerate(top_loss_samples):
       
        img_id = sample["image_id"]
        
        # obtain replace caption
        rep_caption_id = sample["caption_id"] # default use original caption
        if change_caption_by == "sorted_loss" or change_caption_by == "lowest_loss":
            all_captions = caption_dict_by_loss.get(img_id, [])
            if len(all_captions) > 0:
                ...
                if change_caption_by == "sorted_loss" and len(cand_captions) > 0:
                    rep_caption_id = random.choice(cand_captions) 
                elif change_caption_by == "lowest_loss" and len(cand_captions) > 0:
                    rep_caption_id = cand_captions[0]
        
        # get image_path and sd replacement
        # origin2sd: origin_img_path -> List[sd image paths]
        origin_img_path = img2path_dict[img_id]
        sd_images = origin2sd.get(origin_img_path, [])
        
        # obtain replace image id
        source = sample.get("source", "original")
        if len(sd_images) != 0:
            img_path = sd_images[0]
            sd_images.pop(0) # update sd candidates, pop out used ones
            origin2sd[origin_img_path] = sd_images # if could not find an corresponding sd image (or is already sd), stay same
            source = "sd"
        else:
            img_path = origin_img_path
        
        if img_id in original_train_caption_dict and rep_caption_id in original_train_caption_dict.get(img_id, {}):
            text_segment = original_train_caption_dict[img_id][rep_caption_id]["text_segment"]
        else:
            text_segment = sample["text_segment"]
            rep_caption_id = sample["caption_id"]
...
def main(args, ds_init):
    ...
    if not args.eval:
        ...
        if args.curation_method == "replace":
            ...
            lm_img_cap_dict = json.load(open(os.path.join(args.data_path, "flickr30k_train_preprocessed_new_captions_tokenized.json"), "r"))
    else:
        data_loader_train, data_loader_val = create_downstream_dataset(args)
    ...
    if not args.model.endswith(args.task):
        ...
        elif args.task in ("coco_captioning", "nocaps", "flickr30k_captioning"):
    ...
    if args.eval:
        ...
        if args.task in ["nlvr2", "flickr30k", "coco_retrieval", "imagenet"]:
            ...
            if utils.is_main_process() and (args.task == "coco_captioning" or args.task == "flickr30k_captioning"):
                captioning_result = utils.coco_caption_eval(args.data_path, prediction_file, "{}_test".format(args.task))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(f"Start evaluating train data loss on epoch {epoch}")
        if (args.train_eval or args.curation_method is not None) and args.task in ["coco_captioning", "nocaps", "flickr30k_captioning"]:
            # eval train data loss and save to file
            if epoch == 0:
                data_loader_train_eval = create_dataset_by_split(args, split="train", is_train=False)
            else:
                data_loader_train_eval = create_dataset_by_split(args, split="train-curation", is_train=False, items=new_samples)
            train_eval_result = evaluate_train(data_loader_train_eval, model, device, task_handler, original_train_caption_dict)
            train_result_file = utils.save_result(train_eval_result, args.output_dir, 'train_result_epoch%d'%epoch, args, remove_duplicate="sample_id")
            
            if args.distributed:
                dist.barrier()
            
            # obtain top loss samples from train eval 
            top_loss_samples, rest_samples = get_top_loss_samples(train_result_file, args, img2path_dict)
            print("curate on %d top loss samples... " % len(top_loss_samples))

        # print("Get here")
        if args.curation_method == "remove":
            data_loader_train = create_curated_train_dataset(args, rest_samples, is_train=True)
            print("reinit train data loader from %d previous samples" % len(rest_samples))
            # write new train data to file
            new_samples = rest_samples
        ...
        elif args.curation_method == "replaceCap":
            # create new data samples
            replaced_samples, train_caption_dict = replace_top_loss_samples(top_loss_samples, train_caption_dict, original_train_caption_dict, img2path_dict)
        ...
        elif args.curation_method == "replace":
            print("replacing top loss samples with LM generated captions..")
            replaced_samples = lm_replace_top_loss_samples(top_loss_samples, lm_img_cap_dict)
            new_samples = rest_samples + replaced_samples
        ...
        elif args.curation_method == "replaceImg":
        ...
        if data_loader_val is not None:
            if args.task not in ["coco_captioning", "nocaps", "flickr30k_captioning"]:
                ...
                if utils.is_main_process():
                    test_stats = utils.coco_caption_eval(args.data_path, prediction_file, "{}_val".format(args.task))
