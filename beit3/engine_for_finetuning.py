...
class CaptioningHandler(TaskHandler):
    ...
    def train_batch(self, model, data):
        language_masked_pos=data["language_masked_pos"]
        logits, _ = model(
            image=data["image"], 
            text_ids=data["masked_tokens"], 
            padding_mask=data["padding_mask"], 
            language_masked_pos=language_masked_pos, 
            image_id=data["image_id"])
        masked_labels = data["language_tokens"][language_masked_pos.bool()]
        score = torch.max(logits, -1)[1].data == masked_labels
        acc = torch.sum(score.float()) / torch.sum(language_masked_pos)
        return {
            "loss": self.criterion(logits, masked_labels, data["global_step"]),
            "acc": acc
        }
    
    def eval_train_batch(self, model, data):
        model.eval()
        language_masked_pos=data["language_masked_pos"]
        logits, _ = model(
            image=data["image"], 
        ...
        score = torch.max(logits, -1)[1].data == masked_labels
        acc = torch.sum(score.float()) / torch.sum(language_masked_pos)
        loss = self.criterion(logits, masked_labels, 0, reduction=None)
        loss_values = loss.cpu().detach().tolist()
        return {"loss_value": loss_values, "image_id":data["image_id"]}
...
def train_one_epoch(
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if task in ["coco_captioning", "nocaps", "flickr30k_captioning"]:
...
@torch.no_grad()
def evaluate_train(data_loader, model, device, handler, train_caption_dict):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Evaluate train:'
    ...
    for data in metric_logger.log_every(data_loader, 10, header):
        for tensor_key in data.keys():
            if tensor_key not in ["caption_id", "sample_id", "image_path"]:
                data[tensor_key] = data[tensor_key].to(device, non_blocking=True)

        with torch.cuda.amp.autocast():
            results = handler.eval_train_batch(model, data)
            for qid, loss, caption_id, sample_id, img_path in zip(results["image_id"], results["loss_value"], data["caption_id"], data["sample_id"], data["image_path"]):
                eval_records.append({
                    "image_id": qid.item(),
                    "caption_id": caption_id.item(),
                    "loss_val": loss,
                    "text_segment": train_caption_dict[qid.item()][caption_id.item()]["text_segment"],
                    "tokens": train_caption_dict[qid.item()][caption_id.item()]["tokens"],
                    "sample_id": sample_id.item(), #"%d-%d" % (qid.item(), caption_id.item()),
                    "image_path": img_path
                })
    ...
    # with open(os.path.join(train_result_dir, "e%d"%epoch), "w") as output_result_json:
    #     json.dump(eval_records, output_result_json)


    ## TODO: save train records
