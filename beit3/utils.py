...
class BertCaptioningLoss(nn.Module):
    ...
    def forward(self, logits, target, iter, reduction="mean"):
...
def save_result(result, result_dir, filename, args, remove_duplicate=''):
    ...
    if args.distributed:
        ...
        if is_main_process():   
            ...
            print("combining results from all processes...%s" % result_file)
            result = []
            ...
            if remove_duplicate:
                ...
                result_df = result_df.drop_duplicates(subset=[remove_duplicate])       
...
def get_rep_img_path(img_path, origin2sd):
    # obtain sd candidates
    sd_images = origin2sd.get(img_path, [])
    
    # obtain replace image id
    if len(sd_images) != 0:
        rep_img_path = sd_images[0]
        sd_images.pop(0) # update sd candidates, pop out used ones
        origin2sd[img_path] = sd_images # if could not find an corresponding sd image (or is already sd), stay same
    else:
        rep_img_path = img_path
    return rep_img_path

def save_curated_data_into_jsonl(items, jsonl_file, result_dir, args, remove_duplicate='sample_id'):
    file_prefix = os.path.basename(jsonl_file)[:30]
    result_file = os.path.join(result_dir, '%s_rank%d.json'%(file_prefix, get_rank()))
    json.dump(items, open(result_file,'w'), indent=2)
    ...
    if args.distributed:
        ...
        if is_main_process():   
            ...
            for rank in range(get_world_size()):
                result_file = os.path.join(result_dir, '%s_rank%d.json'%(file_prefix,rank))
                print("combining results from all processes...%s" % result_file)
                if os.path.exists(result_file):
                    res = json.load(open(result_file,'r'))
                    result += res
            ...
            # save to final jsonl
            print("Writing %s with %d items !" % (jsonl_file, len(result))) 
            with open(jsonl_file, mode="w", encoding="utf-8") as writer:
                for data in result:
                    writer.write(json.dumps(data, indent=None))
                    writer.write('\n')
            print("Write %s with %d items !" % (jsonl_file, len(result)))         
        return jsonl_file
    else:
        return result_file
...
def coco_caption_eval(gt_dir, results_file, split):
    ...
    filenames = {'coco_captioning_val':'coco_karpathy_val_gt.json',
                 ...
                 'flickr30k_captioning_val': 'flickr30k_val_gt.json',
                 'flickr30k_captioning_test': 'flickr30k_test_gt.json',
                 'nocaps_val':'nocaps_val_gt.json'}
    
    if not os.path.exists(os.path.join(gt_dir, filenames[split])):
